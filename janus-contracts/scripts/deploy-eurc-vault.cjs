...
async function deploy() {
  console.log(`Deploying EURC JanusVault using deployer address: ${account.address}`);
  
  // Verify deployer has balance
  const balance = await publicClient.getBalance({ address: account.address });
  console.log(`Deployer Wallet Balance: ${parseFloat(balance.toString()) / 1e6} USDC/ETH`);
  ...
  const hash = await walletClient.deployContract({
    ...
    args: [
      '0x89B50855Aa3bE2F677cD6303Cec089B5F319D72a', // Underlying asset (Arc EURC)
      '0x59e2532e40982e4233b2cec2d074ad9e6a120f00', // Initial Keeper address (Circle SDK)
      '0x0000000000000000000000000000000000000000', // MultiSig
      '0x0000000000000000000000000000000000000000' // Insurance Fund
    ]
  });
  ...
  console.log(`\n🎉 SUCCESS! EURC JanusVault deployed to Arc Testnet!`);
  console.log(`EURC Vault Address: ${deployedAddress}`);
}
