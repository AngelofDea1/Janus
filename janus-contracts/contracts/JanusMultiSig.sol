...
contract JanusMultiSig is Ownable {
    ...
    mapping(bytes32 => mapping(address => bool)) public hasApprovedPause;
    ...
    function emergencyPauseVault(string memory reason) external {
        ...
        // Count approvals for this pause action (grouped by hour to prevent stale pauses)
        bytes32 pauseId = keccak256(abi.encodePacked("PAUSE", block.timestamp / 1 hours));
        
        require(!hasApprovedPause[pauseId][msg.sender], "Already approved pause");
        
        hasApprovedPause[pauseId][msg.sender] = true;
        pauseApprovals[pauseId]++;
        
        if (pauseApprovals[pauseId] >= REQUIRED_SIGNATURES) {
            vaultPaused = true;
