...
contract JanusVault is ERC4626, Ownable {
    ...
    constructor(
        ...
        require(address(_asset) == ARC_USDC || address(_asset) == EURC, "Must use native Arc USDC or EURC");
    ...
    /**
     * @notice Mint with vault cap check
     */
        ...
        returns (uint256)
    {
        ...
        uint256 actualShares = super.mint(shares, receiver);
        _logAudit("Deposit", receiver, assets);
    ...
    function completeWithdrawal(uint256 requestId) external {
        ...
        // Transfer pre-calculated assets
        IERC20(asset()).safeTransfer(msg.sender, req.assets);
        
        emit WithdrawalCompleted(requestId, msg.sender, req.assets);
        _logAudit("WithdrawalCompleted", msg.sender, req.assets);
    }
    
    /**
     * @notice Harvest yield for the vault
     */
    function harvestYield(
        uint256 amount
    ) external onlyKeeper vaultNotPaused {
        ...
        if (insuranceFee > 0 && insuranceFund != address(0)) {
            IERC20(asset()).forceApprove(insuranceFund, insuranceFee);
    ...
    /**
     * @notice Disabled standard ERC4626 withdraw. Use requestWithdrawal instead.
     */
    ...
    /**
     * @notice Disabled standard ERC4626 redeem. Use requestWithdrawal instead.
     */
