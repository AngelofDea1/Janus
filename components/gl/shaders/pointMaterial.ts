    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          float alpha = (1.5 - clamp(vDistance, 0.0, 1.0)) * clamp(smoothstep(-0.5, 0.25, vPosY), 0.0, 1.0) * uOpacity * revealMask * uRevealProgress * sparkleBrightness;

          gl_FragColor = vec4(uColor, mix(clamp(alpha, 0.0, 1.0), sparkleBrightness - 1.1, uTransition));
        }
      `,
      uniforms: {
