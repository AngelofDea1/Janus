...
async function fetchTokenBalance(tokenAddress: string, walletAddress: string) {
  try {
    ...
    if (!res || !res.ok) {
      throw new Error("RPC_RATE_LIMIT_EXCEEDED: Failed to fetch balance on Vercel after retries");
    }
    const json = await res.json();
    if (json.result && json.result !== "0x") {
      return BigInt(json.result);
...
export async function POST(req: Request) {
  try {
    ...
    const expectedSecret = process.env.CRON_SECRET || process.env.KEEPER_CRON_SECRET;
