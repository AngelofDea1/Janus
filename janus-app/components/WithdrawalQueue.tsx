...
export default function WithdrawalQueue({ 
  ...
  activeVaultAddress: `0x${string}`;
  recentTxHash?: `0x${string}`;
}) {
 ...
 React.useEffect(() => {
   if (!address || !publicClient || !activeVaultAddress) return;
   
   const fetchRequests = async () => {
     try {
       const logs = await publicClient.getLogs({
         address: activeVaultAddress,
         event: parseAbiItem('event WithdrawalRequested(uint256 indexed requestId, address indexed user, uint256 shares)'),
         args: { user: address },
         fromBlock: 0n,
       });

       const reqs = await Promise.all(logs.map(async (log) => {
       ...
       setRequests(reqs.sort((a,b) => b.requestId - a.requestId));
     } catch (err) {
 ...
 }, [address, publicClient, activeVaultAddress, recentTxHash]);
